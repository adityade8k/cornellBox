...
// Your original camera transform (position *and* rotation) is the base.
// Set the position you want, and we’ll capture the quaternion AFTER we aim the camera once.
...
// SpotLight (the “flame”)
const flame = new THREE.SpotLight(0xffb27a, 6.0, 100.0, THREE.MathUtils.degToRad(30), 2.0, 2.5);
flame.position.set(0, -0.12, -1.0);
...
// A real Object3D target so the light can aim correctly
const flameTarget = new THREE.Object3D();
flameTarget.position.set(0, -0.1, 0); // roughly the screen center
scene.add(flameTarget);
flame.target = flameTarget;

lightGroup.add(flame);

// Optional fill light (soft directional)
const directionalLight = new THREE.DirectionalLight(0xffffff, 20);
directionalLight.position.set(0, 0.1, -5);
directionalLight.castShadow = true;
scene.add(directionalLight);

// ---------- Constraints for puppet motion ----------
const constraints = {
  baseZ: -0.9,
  zMin: -1.18,
  zMax: -0.6,
...
// ---------- Load a background GLB (optional) ----------
  ...
  return new Promise((resolve, reject) => {
    loader.load(
      ...
      (tex) => {
        ...
        p.position.set(xOffset, -0.05, constraints.baseZ);
        p.castShadow = true;
...
Promise.all([
  ...
  // After the scene is built, define the camera's BASE orientation:
  // Aim once at the screen (your desired original look direction), then capture baseQuat.
  camera.lookAt(screen.position);
  camera.updateMatrixWorld(true);
  baseQuat.copy(camera.quaternion);         // <<-- capture your *original* rotation as base
...
// ---------- Gyro → camera rotation (relative to base) ----------
const gyroConfig = {
  maxPitchDeg: 2,       // camera X rotation clamp
  maxYawDeg:   2,       // camera Y rotation clamp
  pitchTiltRangeDeg: 15,
  yawTiltRangeDeg:   15,
  smoothing: 0.15       // slerp factor
};
...
const baseQuat = new THREE.Quaternion();  // filled after scene/camera setup
...
function updateTargetFromTilt(beta, gamma) {
  ...
  // Apply small pitch (X) then yaw (Y) ON TOP OF your original camera orientation (baseQuat)
...
function onOrientation(e) {
  ...
  if (!haveBaseline && e.beta != null && e.gamma != null) {
    // capture neutral device tilt values once
    beta0  = e.beta;
    gamma0 = e.gamma;
    haveBaseline = true;
  }
...
// Secure context required on iOS (HTTPS or localhost)
ensurePermissionsIfNeeded();

// ---------- Puppet & light animation ----------
...
function animate() {
  ...
  flame.color.set(0xffb27a).lerp(new THREE.Color(0xffd080), 0.5 * (1 + warmShift));
  ...
  // Puppet motion + occasional twirl
  ...
  // Keep light aimed to the screen center
  flameTarget.position.copy(screen.position);
  flameTarget.updateMatrixWorld();

  // Gyro: slerp camera toward targetQuat (computed from device tilt) WITHOUT changing position
